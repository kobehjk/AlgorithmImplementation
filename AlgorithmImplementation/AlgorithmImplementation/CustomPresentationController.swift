...
//        presentationWrapperView.layer.shadowPath = UIBezierPath.init(rect: CGRect.init(x: (presentationWrapperView.frame.origin.x), y: (presentationWrapperView.frame.origin.y)-6, width: (presentationWrapperView.frame.size.width), height: (presentationWrapperView.frame.size.height))).cgPath
...
//        presentationWrapperView.layer.shadowOffset = CGSize(width: 0, height: -6)
        presentationWrapperView.backgroundColor = #colorLiteral(red: 0.5568627715, green: 0.3529411852, blue: 0.9686274529, alpha: 1)
        ...
        let presentationRoundedCornerView = UIView(frame: UIEdgeInsetsInsetRect((self.presentationWrappingView?.bounds)!, UIEdgeInsetsMake(0, 0, -CORNER_RADIUS, 0))) // 添加圆角效果
...
//        presentedViewControllerView?.addSubview(presentationWrapperView)
        /// 深色的一层覆盖视图，让背景看上去比较暗
